...
def run_medical_experiments(dataset, baseline, params=None, save_model=False, save_results=True, seed=0):
    ...
    if baseline in CONFORMAL_BASELINES:
        train_dataset, calibration_dataset, test_dataset = split_fn(conformal=True, horizon=horizon, seed=seed)
        ...
        train_dataset, _, test_dataset = split_fn(conformal=False, horizon=horizon, seed=seed)
...
def get_uncorrected_medical_results(dataset, seed):
    with open("saved_models/{}-{}-{}.pt".format(dataset, "CFRNN", seed), "rb") as f:
        model = torch.load(f)

    split_fn = DATASET_SPLIT_FUNCTIONS[dataset]
    horizon = HORIZON_LENGTHS[dataset]
    _, _, test_dataset = split_fn(conformal=True, horizon=horizon, seed=seed)

    results = evaluate_cfrnn_performance(model, test_dataset, correct_conformal=False)
    return results
